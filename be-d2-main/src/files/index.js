...
filesRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const url = `http://https://be-d2-production.up.railway.app/${fileName}`;
      ...
      if (oldAuthorIndex !== -1) {
        ...
        res.send("The image file has been uploaded");
      } else {
...
filesRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const url = "http://https://be-d2-production.up.railway.app/" + "blogsCoversImgs/" + fileName;
